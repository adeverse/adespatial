...
#' @param coords A three columns matrix or data frame. Colums 1: identifiers of the points (needs to be numeric). Columns 2 and 3: the X and Y coordinates of the points.
#' @param link A two columns matrix. Each row define an edge. Column 1: The site from which the edge starts. Column 2: the site to which the edge ends. All values in \code{link} need to be integers. 
#' @param unit.angle Character. The measurement units in which the angle is defined: either "degrees" (default) or "radians". 
#' @param rot.angle Numeric. Angle of the vector describing the process influencing the sites. This argument generate a rotation of the site coordinates. The set of coordinates is rotated counterclockwise. Negative values will produce a clockwise rotation.
#' @param rm.same.y Logical (\code{TRUE}, \code{FALSE}). Determines if the links perpendicular to the gradient should be removed. Default value: \code{TRUE}. If these links have already been removed this argument put to \code{TRUE} will make the function crash. See detail for more information. 
...
#' @return
#'
#' \item{\code{se.mat}}{A binary (n x k) matrix of site (n rows) by link edges (k columns).}
#' \item{\code{edges}}{A matrix describing the link edges. It has 2 columns (from, to) and as many rows as there are edges. The edges linked to the fictitious site of origin are found at the beginning of the list.}
#'
#' @references
#'
...
#' str(bin.mat)
#' 
#' ### Build a binary site-by-link matrix using the argument link: remove the site which have identical Y coordinate (by default argument: rm.same.y = TRUE)
