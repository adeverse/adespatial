...
#' the spatial weighting matrix (W matrix) provided (\code{listw}). Then it tests whether the 
...
#' spatial weighting matrix (W matrix), which generates n - 1 predictors. The eigenvalue of the
...
#' The choice of the W matrix is a crucial step when
...
#' \code{\link{MEM.modsel}} to build a set of candidate W matrices and to optimize the choice
#' of the best W matrix, respectively).
...
#' eigenvectors, once the choice of a W matrix was made. The selection of a subset of spatial
...
#' Dray et al. 2006, Blanchet et al. 2008b, Peres-Neto and Legendre 2010, 
...
#' given W matrix using the MIR selection, the selection of a best W matrix can be done on the 
#' basis of the MIR criterion with the function \code{\link{MEM.modsel}}.
#' 
