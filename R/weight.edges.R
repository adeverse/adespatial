...
#'
#' @description These functions construct a vector of weights that can be associated to the edges of the connexion diagram used as a basis to build AEM eigenfunctions. \code{weight.edges} is general and can be used for 1 or 2 dimensional problems. \code{weight.time} is meant to be used only for time series. It is a wrapper for \code{weight.edges}.
#'
#' @param dates A vector of dates, class 'numeric' or 'Date'.
#' @param nb.object Object with class 'nb', computed by the spdep package, containing a list of neighbours for each sampling unit (site or time).
#' @param coords A three-column matrix or data frame. Column 1: identifiers of the points (must be numeric). Columns 2 and 3: the X and Y coordinates of the points.
#' @param distmat Class 'matrix' or 'dist' object containing a dissimilarity or distance matrix. (See details). 
...
#' @param max.d Numeric. Maximum distance for weighting. Default value if max.d=NULL: the maximum distance among a set of sites divided by 2 or the full span of a time series divided by 2 (not recommended in most problems, see details). A warning is given if \code{max.d = NULL} and the default value is used.
#' @param unit.angle Character. The measurement units in which the angle is defined: either "degrees" (default) or "radians". 
#' @param rot.angle Numeric. Angle of the vector describing the process influencing the sites. This argument generates a rotation of the site coordinates. The set of coordinates is rotated counterclockwise. Negative values will produce a clockwise rotation.
#' @param rm.same.y Logical (\code{TRUE}, \code{FALSE}). Determines if the links perpendicular to the gradient should be removed. Default value: \code{TRUE}. If these links have already been removed, this argument put to \code{TRUE} will make the function crash. See details for more information.
#' @param plot.connexions Logical (\code{TRUE}, \code{FALSE}). Determines if the sites and the associated connexion diagram should be plotted after rotation of the coordinates by \code{gradient.angle}.
#'
#' @details
#'
#' These functions should be used in close relationship with \code{\link{build.binary}}, consequently many of the arguments in this function and in \code{\link{build.binary}} are the same.
#' 
#' The argument \code{distmat} may contain general forms of dissimilarity, for example the difficulty of transfer of individuals, matter or energy among the sampling units through space or time. 
#' 
#' In \code{weight.edges}, two weighting functions, described in Legendre and Legendre (2012, eqs. 114.3 and 14.4) have been implemented, where \eqn{d_{ij}} is the distance between sites \eqn{i} and \eqn{j}:
#' 
#' 	\tabular{ll}{
#' 		\code{Weighting function 1:}
#' 		\tab \eqn{1 - (d_{ij}/max(d))^\alpha}{1 - (d_{ij}/max(d))^alpha}
#' 		\cr
#' 		\code{Weighting function 2:}
#' 		\tab \eqn{1/d_{ij}^\beta}{1/d_{ij}^beta}
#' 	}
#' 
#' Also note that if a value is provided for \code{beta} (that is, if it is not \code{NULL}), weighting function 2 is used regardless of whether \code{alpha} is defined or not.
#' 
#' In most applications, the default value of \code{max.d} is not optimal. A more meaningful solution in many applications is to compute a Moran's I correlogram (for univariate data) or a Mantel correlogram (for multivariate data), and provide the distance where the correlation becomes 0 as the value for max.d.
#'
#' @return
#'
...
#'
#' @seealso \code{\link{build.binary}}, \code{\link[spdep]{sp.correlogram}}, \code{\link[vegan]{mantel.correlog}}
#'
#'
