...
#'   log-chord D, D[ik] = chord D[ik] computed on log(y[ij]+1)-transformed data
#'   (Legendre and Borcard 2018) \item chi-square D, D[ik] = sqrt(y[++]
#'   sum((1/j[+j])(y[ij]/y[i+]-y[kj]/y[k+])^2)) \item species profiles D, D[ik]
#'   = sqrt(sum((y[ij]/y[i+]-y[kj]/y[k+])^2)) }
#'
...
#'   percentage difference D (aka Bray-Curtis), D[ik] =
#'   (sum(abs(y[ij]-y[k,j])))/(y[i+]+y[k+]) or else, D[ik] = (B+C)/(2A+B+C)
#'   \item Ružička D, D[ik] = 1-(sum(min(y[ij],y[kj])/sum(max(y[ij],y[kj])) or
#'   else, D[ik] = (B+C)/(A+B+C) \item coeff. of divergence D, D[ik] =
